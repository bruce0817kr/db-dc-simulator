...
## 7. URL 공유/복원 정책 — **미포함(명확히)**

**결정: `yearlySalaries`(및 고급 임금 시나리오 설정 전반)을 URL에 포함하지 않는다.**
...
- URL 직렬화 추가 금지 (yearlySalaries를 buildShareUrl/parseSearchToFormValues에 넣지 말 것).
