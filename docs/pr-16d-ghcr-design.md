...
`v*` tag는 발행 전에 SemVer core와 prerelease 형식을 검사해 `v1.2.3` 또는 `v1.2.3-rc.1` 형태가 아니면 실패한다. 숫자 prerelease 식별자의 선행 0은 거부하며, Docker tag와의 모호한 변환을 피하려고 `+build` metadata는 지원하지 않는다.
