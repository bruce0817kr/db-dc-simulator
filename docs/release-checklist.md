...
| Container | GitHub Actions `Container image` | PR에서는 build-only, master/tag에서만 GHCR publish |
...
- [ ] 불변 SHA tag 또는 digest를 `DB_DC_SIMULATOR_IMAGE`로 지정해 healthcheck 통과
...
| 2026-07-13 | GHCR 이미지 발행 (PR 16D) | PR build-only, master/tag publish, 불변 SHA·SemVer tag, provenance attestation, compose image override |
